...
class ProgramB:
    ...
    def start_server(self):
        ...
        def server_thread():
            ...
            try:
                server_socket.bind((self.host, self.port_b))
                server_socket.listen(1)
                self.add_message("系统", f"B正在端口{self.port_b}上监听...")
    ...
    def connect_to_a(self):
        ...
        def connect_thread():
            try:
                self.socket_to_a = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                self.socket_to_a.connect((self.host, self.port_a))
                self.connected = True
                self.update_status("已连接到A", "green")
                self.add_message("系统", "成功连接到程序A")
                
                # 启动接收来自A的消息的线程
                receive_thread = threading.Thread(target=self.receive_from_a)
                receive_thread.daemon = True
                receive_thread.start()
                
            except Exception as e:
                self.add_message("系统", f"无法连接到A: {str(e)}，5秒后重试...")
                self.root.after(5000, self.connect_to_a)  # 5秒后重试
        
        thread = threading.Thread(target=connect_thread)
        thread.daemon = True
        thread.start()
    
    def receive_from_a(self):
        """从A接收消息"""
        try:
            while self.connected:
                data = self.socket_to_a.recv(1024).decode('utf-8')
                if not data:
                    break
                    
                try:
                    message_data = json.loads(data)
                    self.add_message("A", message_data['message'])
                except:
                    self.add_message("A", data)
                    
        except Exception as e:
            self.add_message("系统错误", f"接收消息错误: {str(e)}")
            self.connected = False
            self.update_status("连接断开", "red")
            self.root.after(5000, self.connect_to_a)  # 5秒后重试连接
    ...
    def send_message(self, event=None):
        ...
        if not self.connected:
            messagebox.showwarning("警告", "未连接到程序A，无法发送消息")
            return
        ...
        try:
            ...
            # 发送消息
            self.socket_to_a.send(json.dumps(message_data).encode('utf-8'))
            
            # 在界面显示
            self.add_message("B", message)
    ...
    def exit_program(self):
        """退出程序"""
        if self.connected and self.socket_to_a:
            self.socket_to_a.close()
        self.root.quit()
        self.root.destroy()
